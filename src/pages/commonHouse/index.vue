<template>
  <section class="AdvertorialPage">
    ...
    <router-link :to="{path: '/release/sale'}" class="side-right-button">List A Home For Sale</router-link>
  </section>
</template>
...
// import VueRental from '@/components/index/rental.vue'
...
export default {
  data () {
    return {
      ...
      bannerDialog: {},
      bannerParam: {
        bannerUrl: '/static/img/common/productBanner/buy_pho.png',
        left: {
          title: 'Buy',
          titleSub: 'Traditional Transaction',
          description: [
            'Contact the seller yourself',
            'Follow the steps to save money with a safe and faster transaction'
          ]
        },
        right: {
          title: 'Information',
          fkCategoryId: Mock.guideId.commonHouse,
          list: []
        }
      }
    }
  ...
  methods: {
    ...
    _getBannerParam () {
      this.$axios.get(`${this.ljj_path}guide/header-banner?fkCategoryId=${Mock.guideId.commonHouse}&pageSize=20`)
        .then(res => {
          if (res.data.success) {
...
.side-right-button
  ...
  padding: 0 15px
  border-radius: 3px
  transform: rotate(-90deg) translateY(170%) translateX(40.5%)
  &:hover
