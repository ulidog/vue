...
export default {
      ...
      bannerParam: {
        ...
        left: {
          ...
          description: [
            ...
            'Transaction online at no cost'
          ]
        },
        right: {
          title: 'Information',
          fkCategoryId: Mock.guideId.buyRequestSft,
          list: []
        }
      }
    }
  ...
  methods: {
    getBannerParam () {
      this.$axios.get(`${this.ljj_path}guide/header-banner?fkCategoryId=${Mock.guideId.buyRequestSft}&pageSize=20`)
        .then(res => {
          if (res.data.success) {
...
.side-right-button
  ...
  padding: 0 15px
  border-radius: 3px
  transform: rotate(-90deg) translateY(206%) translateX(45.5%)
  &:hover
