<template>
  <section class="content-bar">
    ...
    <router-link :to="{path: '/release/rentIn'}" class="side-right-button">Post A Rental Request</router-link>
  </section>
</template>
<script>
...
export default {
      ...
      bannerParam: {
        ...
        left: {
          ...
          description: [
            ...
            'Bad tenants can\'t rent'
          ]
        },
        right: {
          title: 'Information',
          fkCategoryId: Mock.guideId.rentRequest,
          list: []
        }
      }
    }
  ...
  methods: {
    async getBannerParam () {
      const list1 = await this.getBannerParam1(Mock.guideId.rentRequest)
      const list2 = await this.getBannerParam1(Mock.guideId.rentingHouse)
      this.bannerParam.right.list = [...list1.data.data.rows, ...list2.data.data.rows]
      this.$emit('getBannerParam', this.bannerParam)
    },
    async getBannerParam1 (_id) {
      const rtList = await this.$axios.get(`${this.ljj_path}guide/header-banner?fkCategoryId=${_id}&pageSize=20`)
      return rtList
    },
    setCity (_id) {
      this.cityID = _id
...
.side-right-button
  ...
  padding: 0 15px
  border-radius: 3px
  transform: rotate(-90deg) translateY(180%) translateX(45%)
  &:hover
