...
export default {
  data () {
    return {
      ...
      bannerParam: {
        ...
        right: {
          title: 'Information',
          fkCategoryId: Mock.guideId.buyHouse,
          list: []
        }
      }
    }
  ...
  methods: {
    ...
    _getBannerParam () {
      this.$axios.get(`${this.ljj_path}guide/header-banner?fkCategoryId=${Mock.guideId.buyHouse}&pageSize=20`)
        .then(res => {
          if (res.data.success) {
...
.side-right-button
  ...
  padding: 0 15px
  border-radius: 3px
  transform: rotate(-90deg) translateY(167%) translateX(50%)
  &:hover
