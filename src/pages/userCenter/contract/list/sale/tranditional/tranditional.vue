...
<template>
  <div class="part-container">
    <div class="list-container">
      <div class="info-unit" v-for="(unit, index) in collectionData" :key="index" v-if="collectionData.length > 0">
        ...
        <div class="info" @click="goViewHouseDetails(unit.housesId)">
          ...
          <div class="price">$ {{unit.viewPrice}}</div>
          <div class="location">
...
export default {
  ...
  methods: {
    ...
    fetchTTList () {
      const _this = this
      _this.$axios.get(`${this.WXL_LOCAL_PATH}saleHouseOrder/findMyDealSeller`, {params: {transactionMode: 1, pageNum: _this.pageIndex, pageSize: _this.pageSize}})
        .then(res => {
    ...
    goProcessPage (params) {
      ...
      this.$router.push({
        name: 'process-null',
        query: {ebuyhouse: this.$base64.encode(JSON.stringify({id: params.id, type: 1, types: 0}))},
        params: {id: params.id, type: 1, types: 0}
      })
    },
