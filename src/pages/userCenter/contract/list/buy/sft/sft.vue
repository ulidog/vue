...
export default {
  ...
  methods: {
    ...
    fetchSFTlist () {
      const _this = this
      _this.$axios.get(`${this.WXL_LOCAL_PATH}saleHouseOrder/findMyDealBuyer`, {params: {transactionMode: 2, pageNum: _this.pageIndex, pageSize: _this.pageSize}})
        .then(res => {
          // console.info(res)
    ...
    goProcessPage (params) {
       ...
       * @type 普通交易流程还是SFT 0 => sft 1 => 普通
       * @types 租赁还是购买 0 => 购买 1=> 租赁
       **/
      this.$router.push({
        name: 'process-null',
        query: {ebuyhouse: this.$base64.encode(JSON.stringify({id: params.id, type: 0, types: 0}))},
        params: {id: params.id, type: 0, types: 0}
      })
    },
