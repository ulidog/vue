<template>
  <el-row class="step_22_Views">
    <el-col :span="24" class="header">
      <header class="tips">
        <article class="maskTip">
          <span><img src="/static/img/contract/icons/warningStatus.png" alt="warningStatus"></span>
          You have rejected the buyer's discount application waiting for the buyer's reply
        </article>
      </header>
    </el-col>
        ...
        .then(res => {
          if (res.data.success) {
            ...
            if (res.data.data.orderDickerInfo.reduceReason !== undefined) {
              _this.inputContent = res.data.data.orderDickerInfo.reduceReason
            } else {
