<template>
  <el-row class="step_04_Views">
    ...
    <el-col :span="24" v-if="isSuccessStatus" class="content-ref">
      <span>{{saleHouseOrderRecordingInfo.denialReason}}</span>
    </el-col>
    <el-col :span="24" class="content" v-if="!isSuccessStatus">
      <sell-refuse @refuseFrom="refuseFromChange" :refuseTypeList="refuseTypeList"></sell-refuse>
    </el-col>
      ...
      _this.$axios.get(`${this.WXL_LOCAL_PATH}saleHouseOrder/find_offer_detail`, {params: {saleHouseOrderId: _this.params.id, state: _this.params.oldState ? _this.params.oldState : null}})
        .then(res => {
          if (res.data.success) {
            ...
            _this.saleHouseOrderRecordingInfo = res.data.data.saleHouseOrderRecordingInfo
          }
          // console.info(res)
        })
    }
