...
export default {
  ...
  methods: {
    ...
    getOfferDetails () {
      ...
      _this.$axios.get(`${this.WXL_LOCAL_PATH}saleHouseOrder/find_offer_detail`, {params: {saleHouseOrderId: _this.params.id, state: _this.params.oldState ? _this.params.oldState : null}})
        .then(res => {
          if (res.data.success) {
            console.info(res)
            _this.orderInfo.type = Number(res.data.data.saleHouseOrderInfo.transactionMode) !== 1 ? 0 : 1
    ...
    goOfferPage () {
      if (this.params.state === 192) return
      this.$router.push({
        name: 'process-null',
        query: {ebuyhouse: this.$base64.encode(JSON.stringify(Object.assign({}, this.params, this.orderInfo)))},
        params: Object.assign({}, this.params, this.orderInfo)
