  ...
  <div class="part-container">
    <div class="list-container">
      <div class="info-unit" v-for="(unit, index) in collectionData" :key="index">
        ...
        <!-- <div class="btn-group">
          <el-button @click.stop="putOnSale(unit.houseId)">Put on sale</el-button>
        ...
        <div :class="['btn-group', unit.transactionStatus === 2 ? 'grayscale' : '']">
          ...
          <el-button @click.stop="remove(unit.houseId)">Delete</el-button>
          <span v-show="unit.transactionStatus === 3">Completed</span>
...
export default {
  ...
  'methods': {
    ...
    putOnSale (_id) {
      this.$confirm('Confirmed for sale?', 'Tip', {}).then(() => {
        this.$axios({
          'method': 'post',
    ...
    remove (_id) {
      this.$confirm('Confirm delete?', 'Tip', {}).then(() => {
        this.$axios.post(`${this.SH_HOUSE_PROD_PATH}thouses/delete/house`, {
          'houseId': _id
        })
          .then(response => {
            if (response.data.success) {
