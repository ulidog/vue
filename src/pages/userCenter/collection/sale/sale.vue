...
<template>
  <div class="part-container">
    <div class="list-container">
      <div class="info-unit" v-for="(unit, index) in collectionData" :key="index">
        ...
        <div :class="['btn-group', unit.transactionStatus === 2 ? 'grayscale' : '']">
          ...
          <!-- <span v-show="unit.transactionStatus === 2">Trading</span> -->
    ...
    <!-- <button v-if="collectionData.length > 1" class="batch-delete" @click="batchRemove">
      <img src="/static/img/userCenter/batch_delete.png"/>&ensp;批量删除
    </button> -->
    <el-pagination
...
export default {
  ...
  'methods': {
    getCollectionData () {
      ...
      _this.$axios({
        'method': 'GET',
        'url': `${_this.SH_HOUSE_PROD_PATH}thouses/collection/house`,
        'params': {
        ...
        .then(response => {
          ...
          if (response.data.success) {
            if (data instanceof Array && data.length > 0) {
              ...
              _this.dataTotal = response.data.data.total
              console.log(_this.collectionData)
            } else {
    ...
    remove (_id) {
      this.$confirm('Confirm delete?', 'Tip', {}).then(() => {
        this.$axios({
          'method': 'post',
          // 'url': `http://192.168.0.189:9209/web/thouses/delete/browse/house`,
    ...
    batchRemove () {
      let ids = this.collectionData.map(item => {
        return item.houseId
      })
