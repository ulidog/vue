...
/deep/ .row-housing-type .el-radio {
  width: 25%;
  margin-left: 0;
  line-height: 40px;
}
  ...
  <div class="part-container">
    ...
    <div class="part-body">
      <el-form :model="form" :rules="rules" ref="form" label-width="140px">
          ...
          <el-col :span="8">
            <el-form-item label="Garage">
              <el-select v-model="form.garageCount">
                <el-option
                  v-for="(item, index) in garageData"
                  :key="index"
                  :label="item"
                  :value="item"
                ></el-option>
          ...
          <el-col :span="8">
            <el-form-item label="Bathrooms">
              <el-select v-model="form.bathroomCount">
                <el-option
                  v-for="(item, index) in bathroomData"
                  :key="index"
                  :label="item"
                  :value="item"
                ></el-option>
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="Rental Type" prop="resource">
              <el-radio-group v-model="form.releaseType">
                <el-radio label="1">Whole house</el-radio>
                <el-radio label="2">Single room</el-radio>
              </el-radio-group>
            </el-form-item>
          </el-col>
        </el-row>

        <el-row class="row-housing-type">
          <el-col :span="24">
            <el-form-item label="Housing Type" prop="resource" :style="{marginBottom: 0}">
              <el-radio-group v-model="form.housingType">
                <el-radio
                  v-for="(item, index) in housingTypeData"
                  :key="index"
                  :label="item.id"
                >{{item.name}}</el-radio>
              </el-radio-group>
...
export default {
    ...
    return {
      ...
      rules: {
        cityName: [
          {
            required: true,
            message: 'Please enter the name of city',
            trigger: ['blur', 'change']
          }
        ],
        minPrice: [
          {
            required: true,
            message: 'Please enter the minimum price.',
            trigger: ['blur', 'change']
          }
        ],
        maxPrice: [
        ...
        livingSqft: [
          {
            required: true,
            message: 'Please enter the living area.',
            trigger: ['blur', 'change']
          }
        ]
      }
  ...
  methods: {
    ...
    getHousingTypeData () {
      this.$axios
        .post(`${this.zjc_path}findHouseTypeList`, {})
        .then(response => {
    ...
    getCityData (keyword, cb) {
      this.$axios
        .get(`${this.SH_MAP_PROD_PATH}region/list/Str`, {
          params: {
            keyword: keyword,
            pageNum: 1,
            pageSize: 100
          ...
          if (data.success) {
            if (data.data instanceof Array && data.data.length > 0) {
              ...
              this.cityData = data.data.map(item => {
                return {
                  value: `${item.cityAscii},${item.stateId}`,
                  cityID: item.id,
    ...
    validateForm () {
      ...
      if (!this.form.cityName) {
        return { flag: false, msg: 'Please enter the name of city. ' }
      }
      if (!this.cityData.find(item => item.value === this.form.cityName)) {
        return {
          flag: false,
          msg: 'The city you entered can not be found in the list. '
        }
      }
      // this.form.minPrice = parseInt(this.form.minPrice)
      // if (isNaN(this.form.minPrice)) {
      //   return { flag: false, msg: 'Please enter the minimum price. ' }
      // }
      if (parseInt(this.form.minPrice.toString().replace(/\D/g, '')) < 0) {
        return {
          flag: false,
          msg: 'The minimum price should not be less than 0'
        }
      }
      if (parseInt(this.form.minPrice.toString().replace(/\D/g, '')) > 1000000000) {
        return {
          flag: false,
          msg: 'The maximum price should not exceed 100000000.'
        }
      }
      // // this.form.maxPrice = parseInt(this.form.maxPrice)
      // if (isNaN(this.form.maxPrice)) {
      //   return { flag: false, msg: 'Please enter the maximum price. ' }
      // }
      if (parseInt(this.form.maxPrice.toString().replace(/\D/g, '')) < 0) {
        return { flag: false, msg: 'The maximum price must be more than 0. ' }
      }
      if (parseInt(this.form.maxPrice.toString().replace(/\D/g, '')) > 1000000000) {
        return {
          flag: false,
          msg: 'The minimum price must be less than 100000000.'
        }
      }
      if (parseInt(this.form.minPrice.toString().replace(/\D/g, '')) > parseInt(this.form.maxPrice.toString().replace(/\D/g, ''))) {
        return {
          flag: false,
          msg: 'The minimum price should not exceed the maximum price.'
        }
      }
      this.form.livingSqft = Number(this.form.livingSqft)
      if (isNaN(this.form.livingSqft)) {
        return { flag: false, msg: 'Please enter living areas.' }
      }
      if (this.form.livingSqft < 0) {
        return {
          flag: false,
          msg: 'The living area can\'t be less than 0 square meters.'
        }
      }
      if (this.form.livingSqft > 1000000000) {
        return {
          flag: false,
          msg: 'The living area shall not be more than 1000000000 square meters.'
        }
      }
    ...
    submit () {
      ...
      this.$emit(
        'collectData',
        result.flag ? Object.assign(result, this.form) : result
      )
    },
    priceformat (p) {
