...
<template>
  <div
    class="house-list-layer"
    @click="layerOnClick($event)"
    @mousewheel="layerOnMousewheel($event)"
  >
    <div class="dialog">
      <h6>House list</h6>
      ...
      <div class="btn-group">
        <button class="btn-set-out" @click="sendhouse">Set out</button>
...
export default {
  ...
  methods: {
    ...
    handleCheckAllChange (_info) {
      this.isCheckInp.includes(_info.houseId)
        ? this.isCheckInp.splice(this.isCheckInp.indexOf(_info.houseId), 1)
        : this.isCheckInp.push(_info.houseId)
    },
    sendhouse () {
      ...
      if (isCheckInps) {
        ...
        this.closeLayer()
        this.$message.error(
          'Please choose suitable homes from the recommended lists.'
        )
      }
    }
  },
