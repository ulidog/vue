...
<template>
  <div class="detail-part">
    ...
    <div><span>Buying Request</span></div>

    <div class="price">${{detailInfo.priceMin || 0}}~{{detailInfo.priceMax || ''}}</div>
    ...
    <div class="collect">
      ...
      <img
        ...
        @click="collect(detailInfo.id)"
      />
      <!-- } -->
    </div>

    <div class="location clear">
      <img src="/static/img/common/header/location.png">&ensp;{{detailInfo.cityName || ''}}
    </div>
    <div class="label">
      <el-tag v-for="(label, index) in (typeof detailInfo.labelId == 'string' ?  detailInfo.labelId.split(',') : detailInfo.labelId )" :key="label + index">{{label}}</el-tag>
    </div>
    ...
    <div class="more">
      <router-link to="/buyRequestList" tag="span">More &gt;</router-link>
    </div>

    <vue-house-list v-if="houseListVisibility"
      @setHouseListVisibility="setHouseListVisibility"
      :houseListVisibility="houseListVisibility"
      :detailInfo="detailInfo"></vue-house-list>

  </div>
</template>

<script>
import VueRecommend from '@/components/buyRequestDetail/recommend.vue'
import VuePotentialRenter from '@/components/buyRequestDetail/potentialRenter.vue'
