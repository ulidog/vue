    ...
    <div class="filter-part">
      <table>
        <tbody>
          ...
          <tr>
            <td class="title">Rental type : </td>
            <td>
              <span
                v-for="(type, index) in rentInTypeData"
                :key="type.name + index"
                :class="[{'selected': type.name === selectedRentInTypeName}]"
                @click="setRentInType(type)"
              >
                {{type.name}}
              </span>
            </td>
...
export default {
  ...
  'methods': {
    ...
    setRentInType (_type) {
      this.selectedRentInTypeName = _type.name
      this.$emit('setRentInType', _type.id)
    }
  },
  created () {
    this.getHotCityData()
    ...
    this.setRentInType(this.rentInTypeData[0])
  }
}
</script>
