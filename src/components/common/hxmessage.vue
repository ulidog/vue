  ...
  methods: {
    ...
    sendMessage () {
      ...
      if (!this.msgBold || !(this.msgBold.replace(/&nbsp;|\s/g, ''))) {
        this.$message.info('The message you sent cannot be blank.')
        return
      }
      ...
      return _msg.replace(regImg, function (match, params) {
        ...
        try {
          ...
          msg.set({
            ...
            ext: {
              em_apns_ext: {
                em_push_content: 'You have a image message'
              }
            },
    ...
    autoSendFileChange (__this, _type) {
      ...
      msg.set({
        ...
        ext: {
          ...
          em_apns_ext: {
            em_push_content: `You have a ${_type === 'img' ? 'image' : _type} message`
          }
        },
    ...
    lineServiceChange () {
      this.$axios.get(`${this.ZJC_ADMIN_PROD_PATH}random/customer`)
        .then(res => {
          ...
          this.UPDATE_USER_SERVICE_LIST(res.data.data.map(item => {
    ...
    fetchHistoryList () {
      ...
      // CurrentUser
      this.$axios.get(`${this.XT_PROD_PATH}customer/select/hx/chat`, {params: {
