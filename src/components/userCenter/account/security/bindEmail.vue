...
<template>
  <div>
    <h6 class="title">
      <span class="title-txt">Bind email</span>
      <span class="title-tip">Enter the verification code and you can bind the email.</span>
    </h6>
    <el-form :model="form" :rules="rules" ref="form" label-width="180px">
      <el-form-item label="New email" prop="newEmail">
        <el-input type="text" v-model="form.newEmail" autocomplete="off" placeholder="Enter a new Email number"></el-input>
      </el-form-item>
    ...
    submitForm (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$axios({
            'method': 'post',
            'url': `${this.wxl_path}customer/third/party/insert`,
            'data': {
              'email': this.form.newEmail,
              'validateCode': this.form.newCode
            }
          })
            .then(response => {
              if (response.data.success) {
